...
<%
	...
	// board_id로 유저아이디 얻기
	String userId = bdao.getUser_id(board_id);
	...
	List<String> listImage = idao.boardIdImage(board_id);
	...
	<div class="post-container">
		<div class="wrap">
			...
			<table class="post table table-bordered"  style="width:930px; height:450px; border: 1px solid #efefef;">
				<tr>
					<td rowspan="4" style="padding: 0; width: 300px;">
						<div class="sajin"></div> 
						  <%
      // 쓴글에 이미지가 있을때만 사진영역 출력하기 
      int imageCount = idao.getImageCount(board_id);
      // 보드 아이디의 user_id와 현재 로그인된 user_id 비교 
      
      if(imageCount > 0){     
       %>
        <div class="sajin">
          <div id="wrapper_<%= board_id %>" class="wrapper">
          <input type="hidden" id="imgPos_<%=board_id%>" pos="0" totalSlides="<%= listImage.size() %>" sliderwidth="">
      		<div id="slider-wrap_<%= board_id %>" class="slider-wrap" board_id="<%= board_id %>"  onmouseover="" onmouseout="">
          		<ul id="slider_<%= board_id %>" class="slider">
						
						<%
          for(String str : listImage){
          %>
		        	<li class="imgli_<%= board_id %>">                
						<img src="http://localhost:9000<%= root %>/save/<%= str %>" />	
		            </li> 
         <%  
         }
          %>
        	...
        	<!--controls-->       	 
            <div class="btns next" onclick="slideRight(<%=board_id%>);" id="next_<%= board_id %>"><i class="fa fa-chevron-circle-right fa-2x" aria-hidden="true"></i></div>
            <div class="btns previous" onclick="slideLeft(<%= board_id %>);" id="previous_<%= board_id %>"><i class="fa fa-chevron-circle-left fa-2x"></i></div>
            <div class="counter" id="counter_<%= board_id %>"></div>            
            <!--controls--> 
      ...
      %>
            ...
            if(imageCount > 1){ %>
            <!-- 사진 옆으로 밀때 표시되는 동그라미  -->
            <ul style="margin-top: 0px; margin-left: 100px;">
            <li><span class="pagination-wrap" id="pagination-wrap_<%= board_id %>" style="margin-left: 350px;"> <ul> </ul> </span></li>
            </ul>
		...
		//current position
		$(document).ready(function(){
			addImageSlide(<%= board_id %>, <%= listImage.size()%>);
		});
			...
			pagination(boardId, 0);
					...
					<td style="padding: 0;">
						...
						<div class="post-header" align="right">
							...
							<!-- 프로필사진 -->
							<a href="#" class="profile"> <img src="http://localhost:9000<%= root %>/save/<%= udto.getProfile_img()%>" alt=""
								align=""></a>
						...
						<div class="text-container" align="right">
							...
							<div class="usercomment">
								<a href="#" class="profile"
									...
									src="http://localhost:9000<%= root %>/save/<%= udto.getProfile_img()%>" alt="" align="">
								</a> <a href="#"
							...
							<div class="commentincontent" style="">
								...
								<img style="width: 32px; height: 32px; border-radius: 100%;"
									src="http://localhost:9000<%= root %>/save/<%= udto.getProfile_img()%>" /></span> <b
